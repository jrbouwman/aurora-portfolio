...
### Stand per 13 mei 2026
- **Roadmap Rogers-adopter strategie** (`roadmap.astro` `#wie-wanneer`): SVG bell curve met 4 fase-markers (Pioniers/individueel, AKD/koploper-kantoor, CMS NL/via AKD, Verzekeraars/via premie). Kritieke-massa-marker op ~16% (push→pull omslag). Tab-design met 1-view detail-panel per fase: doelpartij, signaal, doelprofiel, contactpersoon, kanaal, tone-of-voice per fase (nieuwsgierig peer-to-peer / partnerschappelijk / stilte als strategie / zakelijk data-gedreven), pitch, waarom, backup-route. Theorie audit gedaan door sub-agent die Rogers imiteerde - jargon (venturesome, cosmopolite) eruit, fase 4 herstructureerd naar beroepsaansprakelijkheidsverzekeraars als system-changers (geen pitchees). KPI-tijdlijn met duur/investering/omzet per fase, NL-afkortingen (geen ARR).
- **Globale image-lightbox** (`BaseLayout.astro`): klik op elke `<img>` → fullscreen modal met Esc/X/klik-buiten = sluiten. Filtert decoratieve images (opacity-low, pointer-events-none, < 64px, links/buttons). MutationObserver pakt ook dynamisch toegevoegde images. Opt-out via `data-no-lightbox` attribuut.
- **Fase-tabs JS** (`BaseLayout.astro`): generieke handler voor `[data-fase-tab]` buttons + `[data-fase-panel]` panels, hashchange-listener voor `#fase-N` deep-links. Gebruikt door roadmap-pagina.
- **Foto-grids prototyping** (`refine/prototyping.astro`): PELV-1 schetsen herschikt (3-prototypes + NFC-device gestapeld links + notitie portrait rechts), PELV-2 prototype (demo portrait links + vooraanzicht + zijaanzicht gestapeld rechts). EXIF orientation reset op 7 foto's (waren als orientation 6 opgeslagen, pixels al landscape).
- **Verdienmodel ipv "Financieel model"** (`proof-of-value.astro`): TOC + section heading hernoemd. DVF-criterium-refs ongewijzigd.
- **Procesdemo video** (`proof-of-value.astro` `#oplossing`): `/videos/proces-demo.mp4` (10MB, 73s, H.264 CRF 26 op 1280px), poster-frame als JPG. Autoplay bij scroll-in-view via IntersectionObserver (muted, playsinline, pauzeert wanneer uit beeld). Onder de video twee blokjes: "Voor de eindklant" (pincode = grootste verandering) en "Voor de medewerker" (verandering nog kleiner) + Rogers-compatibiliteit-zin met klikbare bron. Direct boven de procesflow-SVG dus twee-sporen-presentatie (video + diagram).
...
- `public/videos/proces-demo.mp4` — 73s demo van het verificatieproces vanuit gebruikersperspectief (10MB, H.264 CRF 26 op 1280px, faststart). Gecomprimeerd uit `~/Downloads/Hihi scannen ID.mp4` (186MB origineel). Op PoV-pagina autoplay-on-scroll. **Niet via python-pptx round-trippen.**
- `public/videos/proces-demo-poster.jpg` — poster-frame (35KB, frame op 2s) voor het video-element
